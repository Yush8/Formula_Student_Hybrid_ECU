...
## 11. A bare `make` in `CM7/Debug` DELETES the build

**Symptom.** You run `make` to check something compiles, and instead every object
file and the `.elf` disappear.

**Cause.** CubeIDE's generated `makefile` `-include`s each `subdir.mk` *before*
it declares `all:`, and those fragments declare `clean:` first. GNU make takes
the **first target it sees** as the default goal, so a bare `make` runs `clean`.
CubeIDE always passes an explicit target, so it never trips over this.

**Fix.** Always name the target:

```bash
make all
```

Nothing is lost permanently — `make all` rebuilds it — but a full rebuild of the
HAL costs a few minutes you did not plan to spend.

---

## 12. A new `.c` file is not in the build

**Symptom.** You add a file to `CM7/Core/Src/` and the link fails with
`undefined reference` to everything in it, even though the file itself compiles.

**Cause.** The build's file list lives in `CM7/Debug/Core/Src/subdir.mk` and
`CM7/Debug/objects.list`, both **generated by CubeIDE**. It regenerates them when
it notices a new file in a source folder — but it only notices on a refresh.

**Fix.** In CubeIDE: select the project, press **F5** (Refresh), then build.
Editing `subdir.mk` by hand works but CubeIDE will overwrite it, so use F5.

This applies to `CM7/Core/Src/events.c`, added with the event recorder.

---

## 13. Things that look like bugs but are decisions

...
- **[EXTENDING_THE_HCU.md](EXTENDING_THE_HCU.md)** — the Simulink-only guide: how to
  add a tunable parameter, a CAN message, a logged signal, a live-telemetry
  signal or an I/O pin, all as one `.def` line. **Start here to add anything.**
...
- `ConfigGUI.py` — USB console GUI, three tabs: **Live Telemetry** (watch every
  model signal update live), **Config** (auto-discovered tunable parameters),
  **Console** (raw commands + `stats`). Set the clock, save/defaults,
  auto-reconnect. `pip install pyserial; python ConfigGUI.py`.
